...
# Initial thread count and the max limit for threads
current_threads = 128  # Start with 64 threads
max_threads = 128  # Max number of threads
min_threads = 8  # Min number of threads
error_threshold = 2  # Max errors before reducing threads
...
def secondLastStage_tx(tx):
    # Final cleanup to merge relationships if there are duplicates
    query = """
    MATCH (a:Page)-[l:LINKS]->(b:Page)
    WITH a, b, COLLECT(l) AS links
    WHERE SIZE(links) > 1  // Only look at nodes that have duplicate relationships
    WITH a, b, links[1..] AS duplicates  // Keep the first relationship, delete the rest
    FOREACH (rel IN duplicates | DELETE rel)
    """
    tx.run(query)

#def lastStage_tx(tx):
#add the extra relationships from duplicates_sorted
